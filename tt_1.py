...
def split_image(pic_path):
    ...
    pic_target = 'result_0_'  # 分割后的图片保存的文件夹
    # 要分割后的尺寸
    # cut_width = 50  # 512
    # cut_length = 50  # 512
    cut_width = 60  # 512
    ...
    for i in range(0, num_width):
        for j in range(0, num_length):
            ...
            data = (i + 1) * (j + 1)
            cv2.imwrite(os.path.join('D:/code/bishe/result_0', result_path), pic)
...
# 叠加求平均
def add_all_average(num):
    image_add = [0, 0, 0]
    pic_target = 'D:/code/bishe/result_0'
    file_list = os.listdir(pic_target)
    for count in range(0, len(file_list)):
        im_name = file_list[count]
        im_path = os.path.join(pic_target, im_name)
        image = cv2.imread(im_path)
        image = image.astype(np.float32)
        image_add = image + image_add
    image_add_average = image_add / num
    image_add_average = image_add_average.astype(np.uint8)
    cv2.imwrite('mode.jpg', image_add_average)


def compare_all(num):
    pic_target = 'D:/code/bishe/result_0'
    file_list = os.listdir(pic_target)
    image = Image.open('mode.jpg')
    flag = 0
    f = 0
    for count in range(0, len(file_list)):
        im_name = file_list[count]
...
if __name__ == "__main__":
    # shutil.rmtree('result_0')
    # os.mkdir('result_0')
    # number = run('D:/code/bishe/test/sp_1/002.jpg')
    # # number = run('test1.jpg')
    # add_all_average(number)
    # compare_all(number)

    target = 'D:/code/bishe/test/au_1'
